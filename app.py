...
def save_game_result():
    ...
    # Here you could save individual game results if needed
...
@app.route('/')
def serve_react_app():
    """Serve React app or simple HTML if build doesn't exist"""
    if os.path.exists('frontend/build/index.html'):
        return send_from_directory('frontend/build', 'index.html')
    else:
        # Serve simple HTML if React build doesn't exist
        return '''
        <!DOCTYPE html>
        <html lang="ja">
        <head>
            ...
            <style>
                ...
                .game-area { border: 2px solid #ddd; padding: 20px; margin: 20px 0; border-radius: 10px; background: white; }
                .hidden { display: none; }
        ...
        <body>
            ...
            <div id="app">
                ...
                <div id="game-screen" class="hidden">
                    <h2 id="game-title">今日のゲーム</h2>
                    <p id="game-description"></p>
                    <div class="game-area">
                        <div id="game-content"></div>
                    </div>
                    <button class="button" onclick="backToMain()">メインに戻る</button>
                </div>
            </div>
            ...
            <script>
                ...
                function showGameScreen() {
                    ...
                    document.getElementById('game-title').textContent = selectedGame.name;
                    document.getElementById('game-description').textContent = selectedGame.description;
                    
                    let gameContent = '';
                    if (selectedGame.id === 'catching') {
                        gameContent = '<p>星をクリックしてキャッチしよう！</p><div style="text-align: center;"><button onclick="playGame()" class="button success">ゲームスタート</button></div>';
                    } else if (selectedGame.id === 'memory') {
                        gameContent = '<p>同じ色のカードをマッチさせよう！</p><div style="text-align: center;"><button onclick="playGame()" class="button success">ゲームスタート</button></div>';
                    } else if (selectedGame.id === 'drawing') {
                        gameContent = '<p>自由にお絵かきしよう！</p><div style="text-align: center;"><button onclick="playGame()" class="button success">ゲームスタート</button></div>';
                    }
                ...
                function playGame() {
                    ...
                    alert('ゲーム完了！スコア: ' + score);
                    
                    fetch('/api/game/result', {
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
                        body: JSON.stringify({ score: score, game_id: selectedGame.id })
                    });
